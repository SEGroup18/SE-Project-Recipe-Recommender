...
/**
 * DietPlan component to display healthy recipes based on calorie range.
 * Users can adjust the slider to filter recipes by caloric content.
 * 
 * @returns {JSX.Element} Rendered DietPlan component.
 */
export const DietPlan = () => {
    ...
    // Fetch minimum and maximum calorie values from the server on mount.
    useEffect(() => {
    ...
    // Fetch recipes from the server whenever min or max calories change.
    useEffect(() => {
        server
        .get("/recipe/dietPlan")
        .then((data) => {
    ...
    // Filter recipes based on the selected calorie range.
    useEffect(() => {
    ...
    /**
     * Handles the slider change event to update calorie range.
     * 
     * @param {Event} event - The event triggered by the slider.
     * @param {number[]} newValue - The new calorie range selected by the slider.
     */
 ...
 * @returns {JSX.Element} Rendered Recipe component.
 */
const Recipe = ({ recipe }) => {
    return (
        <Box
